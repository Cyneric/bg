...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if(!empty($_POST['user']) && !empty($_POST['email']) && !empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['pass'])){
		...
		$con = mysqli_connect("localhost","cynericsql2","fe56722c","cynericsql2");
...
<head>		
	...
	<script>
		$(document).ready(function (){
    		$('#user, #email, #firstname, #lastname, #pass').keyup(validate);
		});
                ...
                <div class="panel-body">
                	<form action="register.php" method="post" id="signupform" class="form-horizontal" role="form">          
	                    ...
	                    <div class="form-group">
	                        ...
	                        <div class="col-md-9">
	                        	<input type="text" class="form-control" id="user" name="user" placeholder="Benutzername" value="<?php if(!empty($_POST['user'])) echo $_POST['user']; ?>" required>
	                        </div>
	                   	</div>                
	                    <div class="form-group">
	                    	<label for="email" class="col-md-3 control-label">Email</label>
	                        <div class="col-md-9">
	                        	<input onchange="eval();" type="email" class="form-control" id="email" name="email" placeholder="Email Addresse" value="<?php if(!empty($_POST['email'])) echo $_POST['email']; ?>" required>
	                        </div>
	                   	</div>               
	                    <div class="form-group">
	                    	<label for="firstname" class="col-md-3 control-label">Vorname</label>
	                        <div class="col-md-9">
	                        	<input type="text" class="form-control" id="firstname" name="firstname" placeholder="Vorname" value="<?php if(!empty($_POST['firstname'])) echo $_POST['firstname']; ?>" required>
	                        </div>
	                    </div>
	                    <div class="form-group">
	                       <label for="lastname" class="col-md-3 control-label">Nachname</label>
	                    	<div class="col-md-9">
	                        	<input type="text" class="form-control" id="lastname" name="lastname" placeholder="Nachname" value="<?php if(!empty($_POST['lastname'])) echo $_POST['lastname']; ?>" required>
	                        </div>
	                    </div>
	                    <div class="form-group">
	                    	<label for="pass" class="col-md-3 control-label">Passwort</label>
	                        <div class="col-md-9">
	                        	<input type="password" class="form-control" id="pass" name="pass" placeholder="Passwort" value="<?php if(!empty($_POST['pass'])) echo $_POST['pass']; ?>" required>
	                        </div>
	                    </div>
	                    <div class="form-group">                                     
	                    	<div class="col-md-offset-3 col-md-9">
	                        	<button id="btn-signup" type="submit" class="btn btn-info" disabled><i class="icon-hand-right"></i> &nbsp Registrieren</button>
	                        </div>
	                    </div>       
               		</form> 
            	</div>
...
<?php } ?>  
>>>>>>> 464288a73b4b75de271d997e87cecf63010a018a
